...
if CLIENT then
    ...
    function GAMEMODE:GetNextStepTime(client, vel)
        ...
        if client:WaterLevel() >= 1 then
            ...
            if client:KeyDown(IN_WALK) and vel < 90 then
                ...
                val = math.max(math.Remap(vel, 90, 235, 0.5, 0.3), 0.2)
            end
        end
    ...
    function GAMEMODE:ModifyPlayerStep(client, data)
        ...
        if character then
            ...
            if class and class.ModifyPlayerStep then
                class:ModifyPlayerStep(client, data)
            end

            local faction = ix.faction.Get(character:GetFaction())
            if faction and faction.ModifyPlayerStep then
                faction:ModifyPlayerStep(client, data)
            end
        end
    ...
    function GAMEMODE:HandlePlayerStep(client, side)
        ...
        local stealth = char:GetAttribute("stealth", 0) or 0
        if stealth > 0 then
            volume = volume * ( 1 - stealth / ix.config.Get("maxAttributes") )
        end
