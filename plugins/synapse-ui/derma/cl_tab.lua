...
function PANEL:Init()
    ...
    self.buttonSize = self:GetWide() * 0.04
    self:SetRightOffset(self.buttonSize + self:GetPadding() * 2)

    -- main button panel
    self.buttons = self:Add("Panel")
    self.buttons:SetSize(self.buttonSize, self:GetTall() - self:GetPadding() * 2)
    self.buttons:Dock(RIGHT)
    self.buttons:SetPaintedManually(true)

    local close = self.buttons:Add("synapse.button")
    close:SetText("")
    close:SetSize(self.buttons:GetWide(), self.buttons:GetWide())
    close:Dock(BOTTOM)
    close.DoClick = function()
        self:Remove()
    end
    close.PaintOver = function(button, width, height)
        surface.SetDrawColor(255, 255, 255, 255)
        surface.SetMaterial(PLUGIN.tabIcons["return"])
        surface.DrawTexturedRect(16, 16, width - 32, height - 32)
    end

    local characters = self.buttons:Add("synapse.button")
    characters:SetText("")
    characters:SetSize(self.buttons:GetWide(), self.buttons:GetWide())
    characters:Dock(BOTTOM)
    characters:DockMargin(0, 0, 0, 8)
    characters.DoClick = function()
        self:Remove()
        vgui.Create("ixCharMenu")
    end
    characters.PaintOver = function(button, width, height)
        surface.SetDrawColor(255, 255, 255, 255)
        surface.SetMaterial(PLUGIN.tabIcons["menu"])
        surface.DrawTexturedRect(16, 16, width - 32, height - 32)
    end
...
function PANEL:TransitionSubpanel(id)
    ...
    if (IsValid(subpanel)) then
        ...
        if (IsValid(lastSubpanel)) then
            ...
            if ( !lastSubpanel.currentAlpha ) then
                lastSubpanel.currentAlpha = 255
            end

            lastSubpanel:CreateAnimation(animationTime / 2, {
        ...
        if ( !subpanel.currentAlpha ) then
            subpanel.currentAlpha = 0
        end

        subpanel:SetVisible(true)
        subpanel:CreateAnimation(animationTime / 2, {
            target = {currentAlpha = 255},
            easing = "outQuint",
...
function PANEL:GetOverviewInfo(origin, angles, fov)
    ...
    local forward = originAngles:Forward() * 64 + originAngles:Right() * 8
    forward.z = 0
    ...
    newAngles.yaw = newAngles.yaw - 25
    newAngles.pitch = -10
    newAngles.roll = 0

    return LerpVector(fraction, origin, newOrigin), LerpAngle(fraction, angles, newAngles), Lerp(fraction, fov, 45), bDrawPlayer
end
...
function PANEL:GetStandardSubpanelSize()
    return ScrW() - self.buttonSize - self:GetPadding() * 3, ScrH() - self:GetPadding() * 2
end
...
function PANEL:SetupTab(name, info, sectionParent)
    ...
    if (sectionParent) then
        ...
        button:SetFont("synapse.nagonia.4")
        ...
        button:SetText("")
        button:SetSize(self.buttons:GetWide(), self.buttons:GetWide())
        button:SetButtonList(self.tabs.buttons)
        button:SetBackgroundColor(buttonColor)
    end

    button.PaintOver = function(button, width, height)
        local icon = PLUGIN.tabIcons[name]
        if (icon) then
            surface.SetDrawColor(255, 255, 255, 255)
