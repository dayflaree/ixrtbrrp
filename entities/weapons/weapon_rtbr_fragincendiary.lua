...
SWEP.ViewModel		= "models/weapons/grenadeincendiary/c_grenade.mdl"
SWEP.WorldModel		= "models/weapons/grenadeincendiary/w_grenade.mdl"
...
SWEP.ReadyTimings	= {
	[ACT_VM_DRAW]				= 12/30,
	-- :)
	[ACT_VM_PULLBACK_HIGH]		= 9999,
	[ACT_VM_PULLBACK_LOW]		= 9999,
}
...
function SWEP:CheckAmmo()
	-- FIXME "attempt to compare boolean with number" when called from line 136
...
function SWEP:Think()
	...
	if self:GetPullTime() ~= -1 and CurTime() > self:GetPullTime() then
		if CurTime() > self:GetNextBlip() and CurTime() < self:GetPullTime() + 3.0 then
			...
			local bwah = 1
			if self:GetElapsedCookTime() > 1.5 then bwah = 0.3 end
...
local blip = Sound("RTBRGrenadeIncendiary.Blip")
...
function SWEP:CreateNade(position, angle, velocity, angvel, timer)
	...
	local frag = ents.Create("rtbr_grenade_fragincendiary")
...
hook.Add("DoPlayerDeath", "RTBRDropLive", function(ply)
	...
	if IsValid(wpn) and wpn:GetClass() == "weapon_rtbr_fragincendiary" and wpn:GetPullTime() ~= -1 and wpn:GetPullTime() < CurTime() then
		wpn:DropLive()
	end
end)
