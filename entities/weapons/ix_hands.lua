...
if (CLIENT) then
    ...
    local CROSSHAIR_MAT = Material("mrp/hud/crossair/crosshair.png", "smooth")
    function SWEP:DoDrawCrosshair(x, y)
        local size = 32
        local alpha = 40 -- default: 16% opacity
        local maxInteractDist = 130 -- Only highlight if within this distance
        ...
        if IsValid(ent) and ent ~= game.GetWorld() then
            ...
            local isProp = class:find("prop_") and class ~= "prop_door_rotating"
            local isDoor = ent.IsDoor and ent:IsDoor()
            local isDoorClass = class == "func_door" or class == "func_door_rotating" or class == "prop_door_rotating" or class == "func_button"
            local isItem = ent.ixItemID ~= nil
            local isWeapon = ent:IsWeapon()
            local isScripted = ent:IsScripted()
            local isAmmo = class:find("ammo")
            local isPlayer = ent:IsPlayer()
            local dist = ply:GetPos():Distance(ent:GetPos())

            if (isDoor or isDoorClass or isItem or isWeapon or isScripted or isAmmo or isPlayer)
                and not isProp
                and dist <= maxInteractDist then
